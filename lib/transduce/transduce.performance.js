const pipe = require('../../cjs/pipe')
const map = require('../../cjs/map')
const filter = require('../../cjs/filter')
const take = require('../../cjs/take')
...
module.exports = function getData() {
  ...
  const takeHalf = arr => take(arr.length / 2)(arr)
  const rTakeHalf = arr => R.take(arr.length / 2)(arr)
  const transducer = pipe(map(add(1)), filter(isEven), takeHalf)
