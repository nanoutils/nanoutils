...
test('returns new list, that includes only common items from both provided lists with no duplicates', () => {
  ...
  expect(intersection([true, false, undefined, 'hello'], [false, undefined, 'a'])).toEqual([false, undefined])
})
...
test('functions is compared by ref', () => {
  ...
  expect(intersection([1, 2, 3, function() { return 1 }], [1, 2, 3, function() { return 1 }])).toEqual([1, 2, 3])
})
