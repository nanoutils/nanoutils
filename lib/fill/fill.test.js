...
test('fills array with a specified value', () => {
  const arr = [1, 2, 3, 4, 5]
  expect(fill('a')(arr)).toEqual(['a', 'a', 'a', 'a', 'a'])
})

test('returns array itself if a value is undefined', () => {
  const arr = [1, 2, 3, 4, 5]
...
test('fills array with a specified value from specified start to end', () => {
  const arr = [1, 2, 3, 4, 5]
