...
const arrify = value => Array.isArray(value)
  ? value
  : [value, value, value]
...
const NESTED_ARR_VALUE_GEN = (_, arrs) => arrs.map(arr => [arr, RANDOM_VALUE_GEN()])
...
const getNestedObjects = (
  times = TIMES_1M,
  fValues = NESTED_OBJ_VALUE_GEN
) => {
  let objS = {}
  let objM = {}
  let objL = {}
...
const getNestedArrays = (
  times = TIMES_1M,
  fValues = NESTED_ARR_VALUE_GEN
) => {
  let arrS = []
  let arrM = []
  let arrL = []
