...
test('checks dev errors', () => {
  expect(() => adjust(1, 2, [3])).toThrowErrorMatchingSnapshot()
  expect(() => adjust(a => a + 1, false, [3])).toThrowErrorMatchingSnapshot()
  expect(() => adjust(a => a + 1, 2, 3)).toThrowErrorMatchingSnapshot()
})
